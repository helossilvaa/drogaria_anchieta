...
export default function Funcionarios() {
  ...
  const [totais, setTotais] = useState({ total: 0, ativos: 0, novos: 0 });
  const [crescimento, setCrescimento] = useState({ ativos: 0, novos: 0 });
  ...
  const fetchFuncionarios = async () => {
    ...
    const ativosAnterior = data.filter((f) => f.status === "ativo" && new Date(f.dataContratacao).getMonth() === mesAnterior.getMonth()).length;
    const novosAnterior = data.filter((f) => new Date(f.dataContratacao).getMonth() === mesAnterior.getMonth()).length;

    setTotais({ total, ativos, novos });
    setCrescimento({
  ...
  const fetchVendas = async () => {
    ...
    data.forEach((v) => {
      ...
      let dia = periodo === "6meses" ? `${dt.getFullYear()}-${String(dt.getMonth() + 1).padStart(2, "0")}` : dt.toISOString().split("T")[0];
